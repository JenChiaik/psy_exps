...
class Event:
    ...
    def STAGE_choice(self, topic_dict:dict, STAGE_index:int, condition_same:bool, duration:int=120):
        '''
        单个相似性操纵阶段。
        - topic_dict: 问题字典 {index : str_content}。
            ...
            while (not finished_0) or (not finished_1):
                ...
                if (not finished_0) and _pressed_0:
                    ...
                    if _pressed_0 == 'LB':
                        ...
                        self.__wait_allay(win=self.win0, text=config.static_text['wait_allay_choice'])
                ...
                if (not finished_1) and _pressed_1:
                    ...
                    if _pressed_1 == 'LB':
                        ...
                        self.__wait_allay(win=self.win1, text=config.static_text['wait_allay_choice'])
    ...
    def STAGE_PGG(self, STAGE_index:int, STAGE_trials:int, 
        ...
        for i in range(STAGE_trials):
            ...
            self.__trial_PGG(STAGE_index=STAGE_index, trial_index=i, 
                             total_token=total_token, multiply=multiply, 
    ...
    def __trial_PGG(self, total_token:int, multiply:int, 
                    STAGE_index:int, trial_index:int, 
                    process_name='trial_PGG', 
        ...
        self.__render_pgg(pgg=pgg, render_win0=True, render_win1=True)
        self.win0.flip(), self.win1.flip()
        ...
        while (not finished_0) or (not finished_1):
            ...
            if (not finished_0) and _pressed_0:
                ...
                if _pressed_0 == 'LB' and pgg.trial_reservation_0 > 0: #增加贡献，减少保留
                    ...
                    self.__render_pgg(pgg=pgg, render_win0=True, render_win1=False, 
                                      show_STAGE_reward=show_STAGE_reward)
                    self.win0.flip()
                    ...
                    finished_0 = True
                    self.__wait_allay(win=self.win0, text=config.static_text['wait_allay_PGG'])
            ...
            if (not finished_1) and _pressed_1:
                ...
                if _pressed_1 == 'LB' and pgg.trial_reservation_1 > 0: #增加贡献，减少保留
                    ...
                    finished_1 = True
                    self.__wait_allay(win=self.win1, text=config.static_text['wait_allay_PGG'])
        ...
        if show_trial_result:
            ...
            self.page_text(wait=5, auto=True, 
                           text_0=dynamic_param['trial_result_0'], text_1=dynamic_param['trial_result_1'], )
        else:
            self.page_text(wait=1, auto=True, 
                           text_0=' ', text_1=' ')
    ...
    def __dynamic_param(self, pgg:PGG):
        ...
        trial_result_0 = (
            f'您贡献了 {pgg.contribution_0} 枚代币\n对方贡献了 {pgg.contribution_1} 枚代币\n\n'
            f'本轮您的收益：￥{pgg.trial_reward_0}\n本轮对方收益：￥{pgg.trial_reward_1}'
            )
        trial_result_1 = (
            f'您贡献了 {pgg.contribution_1} 枚代币\n对方贡献了 {pgg.contribution_0} 枚代币\n\n'
            f'本轮您的收益：￥{pgg.trial_reward_1}\n本轮对方收益：￥{pgg.trial_reward_0}'
            )
    ...
    def __render_pgg(self, pgg:PGG, render_win0:bool, render_win1:bool, show_STAGE_reward:bool=True):
        '''
        实时渲染 PGG 图形界面。
        - pgg: 当前 PGG trial。
    ...
    def __wait_allay(self, win:visual.Window, text:str):
...
if __name__ == '__main__':
    ...
    debug = Event()
    debug.overture()
    debug.STAGE_PGG(STAGE_index=0, STAGE_trials=10, total_token=10, multiply=0.75)
