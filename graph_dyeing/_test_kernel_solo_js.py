...
class Experiment:
    ...
    def overture(self):
        ...
        #                          color='#ffffff')
        self.win_implicit = visual.Window(size=(600,600),allowGUI=True, title='just for flipping.',color='#000000')
        self.win = visual.Window(size=config.res_conductor if self.exp_param['display mode']=='full screen' else (1600,900),
                                 allowGUI=False if self.exp_param['display mode']=='full screen' else True,
                                 color='#ffffff', winType='pyglet')

        self.kb = keyboard.Keyboard()
        self.js = joystick.XboxController(0)
    ...
    def page_operation(self, GS:type=GraphSeries, stage:type=int):
        ...
        self.render_hud()
        self.win.flip()
        states_before = [False] * len(self.js.getAllButtons())

        update = None

        while True:
            states_current = self.js.getAllButtons()
            for i, (j, k) in enumerate(zip(states_current, states_before)):
                if (j) and (not k): #当前按下/True，此前释放/False
                    update = True # 需要执行 .flip() 更新窗口内容
                    states_before[i] = True #将此前状态 (k) 设为 True
                    if self.js.get_right_shoulder(): #向右选择图形
                        if self.selected_shape_index < len(self.shape_list)-1:
                            self.selected_shape_index += 1
                        else:
                            self.selected_shape_index = 0
                    elif self.js.get_left_shoulder(): #向左选择图形
                        if self.selected_shape_index > 0:
                            self.selected_shape_index -= 1
                        else:
                            self.selected_shape_index = len(self.shape_list)-1
                    elif self.js.get_a(): #向前选择颜色
                        if self.selected_color_index > 1:
                            self.selected_color_index -= 1
                        else:
                            self.selected_color_index = len(self.color_list)-1
                    elif self.js.get_b(): #向后选择颜色
                        if self.selected_color_index < len(self.color_list)-1:
                            self.selected_color_index += 1
                        else:
                            self.selected_color_index = 1
                    elif self.js.get_x(): #检查合法性并染色
                        if GS.graph_series[self.selected_shape_index][2] == True:
                            ...
                            pass
                    elif self.js.get_left_thumbstick() and (None not in colors): #检查是否填完所有颜色，写入，翻页
                        self.win.clearBuffer()
                        self.win.flip()
                        return
                    else:
                        pass
                elif (k) and (not j): #当前释放，此前按下。
                    update = False
                    states_before[i] = False #将此前状态 (k) 设为 False

            if update:
                self.render_graph(GS), self.render_hud(), self.win.flip()
            else:
                self.win_implicit.flip()
    ...
    def finale(self):
        ...
        '''
        self.intermezzo('end')
        self.win.close(), self.win_implicit.close()
...
if __name__ == '__main__':
    ...
    from _test_flow_solo_js import launch
