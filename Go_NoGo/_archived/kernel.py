...
class Event:
    ...
    def __trial(self, block_index:int, trial_index:int, 
                stim_obj:str, target:str, time_limit:float):
        '''
        ...
        while self.clock_local.getTime() < time_limit:
            ...
            if pressed:
                if pressed[0].name in config.keys['operation']:
                    self.record(process_name='None', block_index=block_index, trial_index=trial_index, 
                                disp_stim=stim_obj, target_stim=target, 
                                sub_react='yes', sub_RT=self.clock_local.getTime(), note='None')
                    return
            else:
                pass
        ...
        self.record(process_name='None', block_index=block_index, trial_index=trial_index, 
                    disp_stim=stim_obj, target_stim=target, 
                    sub_react='no', sub_RT=self.clock_local.getTime(), note='None')
        return
