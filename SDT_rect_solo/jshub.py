...
'''
手柄输入监测模块，同时支持单人 / 双人输入。
- 忽略掉“按住”行为，检查单只或两只手柄的按键状态。
- 自带一个用于刷新 backend 的图形窗口，无需重复创建。
（没有完全进行封装，用起来麻烦了点，但能用就行，暂时不改了。）
'''
...
class solo_JS:
    ...
    def __init__(self, mode:str='XboxController'):
        ...
        self.win_flipper = visual.Window(size=(400,300), screen=0, color='#000000', 
                                         title='.flip()' ,winType='pyglet')
    ...
    def check_js(self, _js_before:list, interval:float=2) -> list:

        '''
        查询当前手柄按键状态，但如果上次按键状态与这次相同，则忽略重复“按住”操作。
        ...
        def __check(_js_before:list, _js_current:list, 
                    _last_pressed:list=[0,None]):
            '''
            查询单个手柄按键的私有函数。
            - last_pressed: [_last_pressed_time_0，_last_pressed_name_0]
            '''
            ...
            for i,(before, current) in enumerate(zip(_js_before, _js_current)):
                if current and (not before):
                    ...
                    if js.get_y():
                        ...
                        if (self._pressed_time - _last_pressed[0] <= interval)\
                            and (_last_pressed[1] == 'Y'):
                            pressed = 'Y_A'
                            self._pressed_name = None
                            self._pressed_time = 0
        ...
        pressed_0 = __check(_js_before=_js_before, _js_current=_js0_current, 
                            _last_pressed = [self._pressed_time, self._pressed_name])
...
if __name__ == '__main__':
    ...
    _js0_before = [False]*len(solo.js0.getAllButtons())

    while True:

        _pressed_0 = solo.check_js(_js_before=_js0_before)
