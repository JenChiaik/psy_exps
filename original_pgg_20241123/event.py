...
class Event:
    ...
    def STAGE_choice(self, topic_dict:dict, STAGE_index:int, condition_same:bool, intermezzo:str=None, duration:int=120,):
        '''
        单个相似性操纵阶段。
        ...
        if intermezzo:
            ...
            self.page_pic(pic_0=intermezzo, pic_1=intermezzo, wait=2, auto=False)
        ...
        def __trial_PGG(role:int, trial_index:int, 
            ...
            if finished_0 and finished_1: #完成一个 trial
                ...
                if show_trial_result:
                    ...
                    self.page_text(wait=10, auto=True, 
    ...
    def __dynamic_param(self, pgg:PGG):
        ...
        explain_private_box = f' 每投入 1 枚，自己便获得\n￥1.00'

        trial_result_0 = (
            f'您往公共帐户中投入了 {pgg.contribution_0} 枚代币\n'
            f'对方往公共账户投入了 {pgg.contribution_1} 枚代币\n\n'
            f'本轮您的收益：￥{format(pgg.trial_reward_0,".2f")}\n'
            f'        - 计算：￥0.75 × ({pgg.contribution_0} + {pgg.contribution_1}) + ￥1.00 × {pgg.trial_reservation_0}\n'
            f'本轮对方收益：￥{format(pgg.trial_reward_1,".2f")}'
            )
        trial_result_1 = (
            f'您往公共帐户中投入了 {pgg.contribution_1} 枚代币\n'
            f'对方往公共账户投入了 {pgg.contribution_0} 枚代币\n\n'
            f'本轮您的收益：￥{format(pgg.trial_reward_1,".2f")}\n'
            f'        - 计算：￥0.75 × ({pgg.contribution_0} + {pgg.contribution_1}) + ￥1.00 × {pgg.trial_reservation_1}\n'
            f'本轮对方收益：￥{format(pgg.trial_reward_0,".2f")}'
            )
    ...
    def calculate_income(self):
        ...
        basic_income = max(self.clock_global.getTime() // 60, 35) #每分钟￥1作为基准被试费，不少于 ￥35
...
if __name__ == '__main__':
    ...
    debug = Event()
    debug.overture()
    debug.STAGE_PGG(STAGE_index=1, STAGE_trials=5, total_token=10, multiply=0.75)
    debug.STAGE_choice(topic_dict=config.topic_dict, STAGE_index=1, 
                       condition_same=True if debug.exp_param['condition_A'] == '1' else False, 
                       duration=5)
    ...
    debug.STAGE_PGG(STAGE_index=2, STAGE_trials=5, total_token=10, multiply=0.75)
    debug.finale()
